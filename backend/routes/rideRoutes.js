 ...
 * NOTE: Booking schema fields are `passenger`, `ride`, `driver` — NOT
 * `passengerId` / `rideId`. Populate calls below use the real field names.
 */
const validateAuthorization = (booking, userId) => {
    const passengerId = booking.passenger?._id || booking.passenger;
    const rideDriverId = booking.ride?.driverId?._id || booking.ride?.driverId;
    const bookingDriverId = booking.driver?._id || booking.driver;

    const isPassenger = passengerId?.toString() === userId.toString();
    const isDriver =
        rideDriverId?.toString() === userId.toString() ||
        bookingDriverId?.toString() === userId.toString();
...
router.get('/download/:bookingId', protect, async (req, res) => {
    ...
    try {
        ...
        // FIXED: populate 'passenger' and 'ride' (actual schema field names)
        ...
        if (!auth.isAuthorized) {
            logger.warn('Unauthorized receipt access attempt', {
                ...
                driverId: booking.ride?.driverId?._id
            });
...
router.get('/preview/:bookingId', protect, async (req, res) => {
    ...
    try {
        ...
        // Fetch booking (FIXED: real field names)
        const booking = await Booking.findById(bookingId)
            .populate('passenger', 'name email phone')
            .populate({
                path: 'ride',
        ...
        // NOTE: driver receives the FULL base fare — platform fee/GST are
        // charged only to the passenger (see Transaction.calculateAmounts,
        // where platformFee / gstOnPlatformFee are hardcoded to 0 for the
        // driver side, and driverNetAmount === baseFare).
        const receiptData = {
            receipt: {
            ...
            transaction: {
                ...
                paymentDate: transaction.capturedAt || transaction.createdAt,
            ...
            driver: {
                name: booking.ride.driverId.name,
                phone: booking.ride.driverId.phone || booking.ride.phoneNumber,
                email: booking.ride.driverId.email,
                vehicle: booking.ride.vehicleNumber
            },
            payment: {
                baseFare: transaction.baseFare || 0,
                serviceFee: transaction.passengerServiceFee || 0,
                gst: transaction.gstOnPassengerServiceFee || 0,
                totalFare: transaction.totalAmount ?? booking.totalFare,
                platformCommission: transaction.platformTotalCommission || 0,
...
router.get('/status/:bookingId', protect, async (req, res) => {
    try {
        ...
        // FIXED: select/populate real field names ('passenger', 'ride')
        const booking = await Booking.findById(bookingId)
            .select('passenger driver ride status')
            .populate('ride', 'driverId')
            .lean();
        ...
        const transaction = await Transaction.findOne({
            bookingId,
            paymentStatus: { $in: RECEIPT_CONFIG.ALLOWED_PAYMENT_STATUSES }
        }).select('paymentStatus capturedAt').lean();
        ...
        res.json({
            ...
            data: {
                ...
                paymentDate: transaction?.capturedAt || null
            }
        });
