    ...
    <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4 mt-3">
      ...
      <div className="space-y-1.5 text-sm">
        ...
        <div className="flex justify-between font-bold text-green-700 border-t border-green-200 pt-1.5 mt-1.5">
          ...
          <span className="text-base">₹{netPerSeat.toFixed(0)}</span>
        </div>
        {parseInt(seats) > 1 && (
          <div className="flex justify-between text-xs text-green-600">
            <span>If all {seats} seats filled</span>
            <span className="font-semibold">₹{totalFull.toFixed(0)}</span>
          </div>
        )}
      </div>
    </div>
...
function RidePreviewCard({ data, user }) {
  ...
  const passengerPays = data.fare ? PaymentCalculator.calculatePassengerTotal(data.fare, 1).totalPassengerPays : 0;
      ...
      <div className="bg-white p-5">
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="text-right flex-shrink-0">
            <p className="text-xl font-black text-blue-600">₹{data.fare || '0'}</p>
            <p className="text-xs text-gray-400">per seat</p>
            <p className="text-xs text-gray-500 mt-0.5">Passenger pays ₹{passengerPays.toFixed(0)}</p>
          </div>
        </div>
        ...
        {data.tollIncluded && (
          <p className="text-xs text-green-600 mt-2 flex items-center gap-1"><span>✓</span> Toll charges included</p>
        )}
        {data.negotiableFare && (
          <p className="text-xs text-blue-600 mt-1 flex items-center gap-1"><span>✓</span> Fare is negotiable</p>
        )}
      </div>
    </div>
  );
...
function RideForm({ onSubmit, isLoading }) {
  ...
  // Auto-fill phone from user profile on mount
  useEffect(() => {
  ...
  // ── Build full form data object (used in preview + submit) ────────────────
  const formData = {
    start, end, date, time,
    ...
    preferences: {
      smokingAllowed, musicAllowed, petFriendly,
      luggageAllowed, womenOnly,
  ...
  const handleNext = () => {
    ...
    if (Object.keys(errs).length) {
      ...
      toast.error(first, { position: 'top-center' });
      return;
    }
    setErrors({});
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...
      // Pricing — controller expects these fields exactly
      fareMode:       'fixed',
      ...
      // Contact — controller requires phoneNumber and address
      ...
      waypoints:         waypoints.filter(w => w.location.trim()).map((w, i) => ({
        location: w.location.trim(), order: i + 1,
      })),
      allowPartialRoute,

      // Preferences — matches Ride schema exactly
      preferences: {
        smokingAllowed,
        musicAllowed,
        petFriendly,
        ...
        <div className="px-5 sm:px-8 pb-8">
            ...
            <div className="space-y-5">
              ...
              <Field label="From" required error={errors.start}>
                <div className="relative">
                  ...
                  <input type="text" placeholder="e.g. Phagwara, Punjab" value={start} onChange={e => { setStart(e.target.value); setErrors(p => ({ ...p, start: '' })); }} className={`${inputCls(errors.start)} pl-10`} />
                </div>
              </Field>
              ...
              <Field label="To" required error={errors.end}>
                <div className="relative">
                  ...
                  <input type="text" placeholder="e.g. Chandigarh" value={end} onChange={e => { setEnd(e.target.value); setErrors(p => ({ ...p, end: '' })); }} className={`${inputCls(errors.end)} pl-10`} />
                </div>
              </Field>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Date" required error={errors.date}>
                  <input type="date" value={date} min={today} onChange={e => { setDate(e.target.value); setErrors(p => ({ ...p, date: '' })); }} className={inputCls(errors.date)} />
                </Field>
                <Field label="Departure time" required error={errors.time}>
                  <input type="time" value={time} onChange={e => { setTime(e.target.value); setErrors(p => ({ ...p, time: '' })); }} className={inputCls(errors.time)} />
                </Field>
              </div>
              ...
              <Field label="Available seats" required error={errors.seats} hint="How many passengers can join? (1–8)">
                <div className="flex items-center gap-4 mt-1">
                  <button type="button" onClick={() => setSeats(s => Math.max(1, parseInt(s) - 1))} className="w-10 h-10 rounded-full border-2 border-gray-300 flex items-center justify-center font-bold text-gray-600 hover:bg-gray-100 transition-colors text-lg">−</button>
                  <input
                  ...
                  />
                  <button type="button" onClick={() => setSeats(s => Math.min(8, parseInt(s) + 1))} className="w-10 h-10 rounded-full border-2 border-gray-300 flex items-center justify-center font-bold text-gray-600 hover:bg-gray-100 transition-colors text-lg">+</button>
                  <span className="text-sm text-gray-400">{seats === 1 ? '1 passenger' : `${seats} passengers`}</span>
                </div>
              </Field>

              {/* Fare */}
              <Field label="Cost per seat (₹)" required error={errors.fare} hint="What you want each passenger to contribute to your fuel cost">
                <div className="relative">
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              <div className="border-t border-gray-100 pt-5">
                ...
                <Field label="Contact number" required error={errors.phoneNumber} hint={user?.phone ? 'Auto-filled from your profile — edit if needed' : 'Passengers will call/WhatsApp this number'}>
                  <input
          ...
          {step === 5 && (
            <div className="space-y-5">
              ...
              <RidePreviewCard data={{ ...formData, notes }} user={user} />
